...
void parseAnswerFromAnswerPacket(struct res_record* answerPacket, struct hostent* queryResult) {
	...
	int s1 = sizeof(answerPacket->NAME_REF);
	int a1 = &answerPacket->NAME_REF;
	int s2 = sizeof(answerPacket->RESOURCE.TYPE);
	int a2 = &answerPacket->RESOURCE.TYPE;
	int s3 = sizeof(answerPacket->RESOURCE.CLASS);
	int a3 = &answerPacket->RESOURCE.CLASS;
	uint32_t ttl = ntohl(answerPacket->RESOURCE.TTL);
